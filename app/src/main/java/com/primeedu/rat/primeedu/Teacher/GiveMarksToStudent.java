...
//        StudentId = getIntent().getStringExtra("id").toString();
    ...
    private void check(){
        ...
        for(int i = 0 ;i<table.getChildCount();i++){
            ...
            if(mark.getText().toString().length()>0 && Float.parseFloat(mark.getText().toString())> Float.parseFloat(maximumMark.getText().toString())){
    ...
    private void checkExistingTerms() {
        String path = "Exams/"+SchoolId+"/";
        DatabaseReference ref = FirebaseDatabase.getInstance().getReference(path);
        Query query = ref;
        query.orderByKey().equalTo(examName.getText().toString())
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if(dataSnapshot.getValue() == null){
                            openDialogS("Term code is not valid. Please Check it again.",0);
                        }
                        else{
                            for(DataSnapshot snapshot: dataSnapshot.getChildren()){

                                ExamDetails examDetails = snapshot.getValue(ExamDetails.class);
                                openDialogS("Examination: "+examDetails.getExamname(),1);
                                //getDataFromTable();
                            }
    ...
    private void openDialogS(String msg, final int types ) {
        AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(this);
    ...
    private void getDataFromTable() {
        ...
        for(int i = 0 ;i<table.getChildCount();i++){
            ...
            final int finalI = i;
            ref.child("number").setValue((mark.getText().toString().length() >0)?mark.getText().toString():"0")
